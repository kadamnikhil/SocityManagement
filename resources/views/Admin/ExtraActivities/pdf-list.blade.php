...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        h1.doc-title { font-size: 12pt; margin: 0 0 6px 0; font-weight: bold; color: #0f172a; }
        .doc-sub { font-size: 9pt; color: #475569; margin: 0 0 10px 0; font-weight: bold; }
        table.summary { width: 100%; border-collapse: collapse; margin: 0 0 12px 0; font-size: 9pt; }
        table.summary th {
            text-align: left;
            font-weight: bold;
            color: #374151;
        ...
        .meta { font-size: 8.5pt; color: #6b7280; margin-bottom: 10px; line-height: 1.35; }
...
<body>
    <div class="pdf-header">
        ...
        <p class="letterhead-sub">Extra activity collection report</p>
        <hr class="header-rule">
    </div>

    <div class="pdf-body">
        <h1 class="doc-title">Extra activity collection</h1>
        <p class="doc-sub">{{ $listLabel }} flats · {{ $payments->count() }} record(s)</p>

        <table class="summary">
            <tbody>
                <tr>
                    <th scope="row">Activity title</th>
                    <td>{{ $extraActivity->title }}</td>
                </tr>
                <tr>
                    <th scope="row">Activity type</th>
                    <td>{{ $extraActivity->type_label }}</td>
                </tr>
                <tr>
                    <th scope="row">Amount per flat</th>
                    <td>Rs. {{ number_format((float) $extraActivity->amount_per_flat, 2) }}</td>
                </tr>
                <tr>
                    <th scope="row">Due date</th>
                    <td>{{ $extraActivity->due_date ? $extraActivity->due_date->format('d M Y') : '—' }}</td>
                </tr>
                <tr>
        ...
        @if($payments->isEmpty())
            <p class="muted">No {{ strtolower($listLabel) }} flats for this activity.</p>
        @else
            <table class="data">
                <thead>
                    <tr>
