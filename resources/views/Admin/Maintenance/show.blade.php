...
@section('content')
<style>
    .maint-detail { font-family: system-ui, -apple-system, "Segoe UI", Roboto, sans-serif; }
    .maint-top { background: linear-gradient(135deg, #0f6bff, #0b4bb3); color: #fff; border-radius: .85rem; padding: 1rem 1.15rem; box-shadow: 0 12px 30px rgba(15, 107, 255, 0.18); }
    .maint-top p { color: rgba(255,255,255,.78); }
    ...
    .maint-panel { border: 0; border-radius: .9rem; box-shadow: 0 10px 28px rgba(15, 23, 42, .08); overflow: hidden; }
    ...
    .wing-code { width: 1.8rem; height: 1.8rem; border-radius: .48rem; display: inline-flex; align-items: center; justify-content: center; background: #e0f2fe; color: #0369a1; font-weight: 900; }
...
<section class="maint-detail">
    <div class="maint-top mb-3">
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
            <div>
                <div class="small text-white-50 fw-bold text-uppercase">Maintenance Details</div>
                <h4 class="fw-bold mb-1">{{ $maintenance->label }}</h4>
                <p class="mb-0">
                    @if($maintenance->amount !== null)
                        Rs. {{ number_format((float) $maintenance->amount, 2) }} per flat
                    @else
                        Amount not set
                    @endif
                </p>
            ...
            <div class="d-flex flex-wrap gap-2">
                <a href="{{ route('admin.maintenance.index') }}" class="btn btn-light btn-sm fw-semibold">
                    <i class="ti ti-arrow-left me-1"></i> Back
                </a>
                <a href="{{ route('admin.maintenance.show', [$maintenance, 'status' => 'unpaid']) }}" class="btn btn-warning btn-sm fw-semibold">
    ...
    <div class="row g-3 mb-3">
        <div class="col-md-4">
            <div class="card stat-card"><div class="card-body"><div class="stat-label">Total flats</div><div class="stat-value">{{ $stats['total'] }}</div></div></div>
        </div>
        <div class="col-md-4">
            <div class="card stat-card"><div class="card-body"><div class="stat-label">Paid</div><div class="stat-value text-success">{{ $stats['paid'] }}</div></div></div>
        </div>
        <div class="col-md-4">
            <div class="card stat-card"><div class="card-body"><div class="stat-label">Unpaid</div><div class="stat-value text-warning">{{ $stats['unpaid'] }}</div></div></div>
        </div>
    </div>

    <div class="card maint-panel">
        <div class="card-body p-3 p-md-4">
            <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
                <div>
                    <h5 class="fw-bold mb-1">{{ $filterLabels[$status] ?? 'All flats' }}</h5>
                    <p class="text-muted small mb-0">Wings are collapsible. Open payment details only when you need transaction ID, note, or receipt upload.</p>
                </div>
                <div class="status-tabs d-flex flex-wrap gap-2">
                    <a href="{{ route('admin.maintenance.show', [$maintenance, 'status' => 'all']) }}" class="btn btn-sm {{ $status === 'all' ? 'btn-primary' : 'btn-outline-primary' }}">All</a>
                    <a href="{{ route('admin.maintenance.show', [$maintenance, 'status' => 'paid']) }}" class="btn btn-sm {{ $status === 'paid' ? 'btn-success' : 'btn-outline-success' }}">Paid</a>
                    <a href="{{ route('admin.maintenance.show', [$maintenance, 'status' => 'unpaid']) }}" class="btn btn-sm {{ $status === 'unpaid' ? 'btn-warning' : 'btn-outline-warning' }}">Unpaid</a>
                </div>
            </div>
            ...
            @else
                <div class="wing-accordion" id="maintenanceWingAccordion">
                    @foreach($wings as $wing)
                        @php
                            $floors = $paymentsByWingFloor->get($wing->id, collect());
                            $wingPayments = $floors->flatten(1);
                            $wingCount = $wingPayments->count();
                            $wingPaid = $wingPayments->where('status', MaintenancePayment::STATUS_PAID)->count();
                            $wingUnpaid = $wingPayments->where('status', MaintenancePayment::STATUS_UNPAID)->count();
                            $collapseId = 'wing-maintenance-'.$wing->id;
                        @endphp
                        @if($wingCount > 0)
                            <section class="wing-item" aria-label="Wing {{ $wing->code }}">
                                ...
                                <div id="{{ $collapseId }}" class="collapse {{ $loop->first ? 'show' : '' }}" data-bs-parent="#maintenanceWingAccordion">
                                    <div class="wing-body">
                                            ...
                                            <div class="floor-row">
                                                ...
                                                <div class="flat-grid">
                                                    @foreach($floorPayments as $payment)
                                                        @php
                                                            $flat = $payment->flat;
                                                            $isPaid = $payment->status === MaintenancePayment::STATUS_PAID;
                                                            $nextStatus = $isPaid ? MaintenancePayment::STATUS_UNPAID : MaintenancePayment::STATUS_PAID;
                                                            $detailsId = 'payment-details-'.$payment->id;
                                                        @endphp
                                                        <div class="flat-card {{ $isPaid ? 'flat-card--paid' : 'flat-card--unpaid' }}">
                                                            ...
                                                            <div class="flat-actions">
                                                                <button class="btn btn-outline-primary btn-sm" type="button" data-bs-toggle="collapse" data-bs-target="#{{ $detailsId }}" aria-expanded="false" aria-controls="{{ $detailsId }}">
                                                                    Details
                                                                </button>
                                                                @if($isPaid)
                                                                    <form method="POST" action="{{ route('admin.maintenance.payments.update', [$maintenance, $payment]) }}">
                                                                        @csrf
                                                                        @method('PATCH')
                                                            ...
                                                            <div class="collapse" id="{{ $detailsId }}">
                                                                <form method="POST" action="{{ route('admin.maintenance.payments.update', [$maintenance, $payment]) }}" enctype="multipart/form-data" class="proof-panel">
                                                                    @csrf
                                                                    @method('PATCH')
                                                                    <input type="hidden" name="status" value="paid">
                                                                    ...
                                                                    <button type="submit" class="btn btn-primary btn-sm w-100 fw-semibold mt-2">
                                                                        {{ $isPaid ? 'Update Proof' : 'Mark Paid With Proof' }}
                                                                    </button>
                                                                </form>
                                                            </div>
                                                        </div>
